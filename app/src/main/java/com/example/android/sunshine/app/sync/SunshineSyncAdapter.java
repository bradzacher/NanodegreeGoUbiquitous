...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks
{
    ...
    public final static  String   WEAR_PATH                      = "/weather/today";
    ...
    private void notifyWear()
    {
        ...
        Cursor cursor = getContext().getContentResolver().query(WeatherContract.WeatherEntry.buildWeatherLocationWithDate(Utility.getPreferredLocation(getContext()), System.currentTimeMillis()),
                                                                COLS, null, null, null);
        if (cursor != null && cursor.moveToFirst())
        {
            // get the data from the cursor
            int    weatherID        = cursor.getInt(cursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
            double temperature_high = cursor.getDouble(cursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
            double temperature_low  = cursor.getDouble(cursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));

            // build an asset from the icon
            ...
            // build a request to send to the wearable
            PutDataMapRequest putDataMapRequest = PutDataMapRequest.create(WEAR_PATH);
            putDataMapRequest.getDataMap().putAsset(WEAR_TEMPERATURE_ICON_ASSET, asset);
            putDataMapRequest.getDataMap().putInt(WEAR_TEMPERATURE_LOW, (int) Math.round(temperature_high));
            putDataMapRequest.getDataMap().putInt(WEAR_TEMPERATURE_HIGH, (int) Math.round(temperature_low));
